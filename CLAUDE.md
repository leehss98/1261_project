...
7. **CRITICAL: At most one vehicle per directed segment**: Since the distance between consecutive intersections is 0.5 mile (equal to a segment), and a vehicle can see another vehicle up to 0.5 mile ahead, only 1 car can occupy any directed segment at any time. This constraint derives from: (a) at most 1 car crosses each intersection per time step, (b) a car cannot move if another car is visible ahead within 0.5 mile, and (c) visibility range equals inter-intersection distance. This must be enforced in both i-group and v-group.
...
## Writing Style (Reports and Documentation)

- Do not use emoji in any report or documentation file
- Do not use em dashes; use colons (after labels/headers), semicolons (between clauses), or commas (for parentheticals)
- Use formal verification terminology in reports: semaphore, critical section, mutual exclusion, invariant, guard condition, liveness, starvation-freedom, two-phase protocol, decision procedure
- Intersections are critical sections; CrossingGrant is a binary semaphore signal; CrossingRequest is a semaphore wait
- Safety property tables use the column header "Mechanism" not "How verified"
- The Design Discussion section uses a dialogue format between i-group and v-group

## Git / Remote

- Remote: `git@github.com:leehss98/1261_project.git` (SSH)
- SSH keys in `~/.ssh`; check filenames only, never read key file contents
- Git user: `vishnu5652 <vishnu_rajagopal@tamu.edu>` (set locally for this repo)
- `CLAUDE.md` and `__pycache__/` are gitignored; do not re-add them

## Implementation Decisions (Phase A)

- `is_right_turn_transition()` added to `common_model.py`; right-turn map: NORTH→EAST, EAST→SOUTH, SOUTH→WEST, WEST→NORTH
- `is_front_blocked()` uses a 15-slot visibility limit (0.5 mile per spec); only the nearest car ahead is checked
- Both groups track `right_turn_violations` independently alongside collisions, red-light, and u-turn violations
- `SafetyReport` in i-group and stats dict in v-group both include `right_turn_violations`

## Files Added This Session

- `README.md` — project overview and run instructions
- `PhaseA_Report.md` — Phase A submission report; fill in Team ID and Members before submitting to Canvas
